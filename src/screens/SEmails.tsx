...
export const EmailsScreen = () => {
    ...
    const handleSendCode = async () => {
        ...
        if (res.success) {
            Toast.show({ type: 'success', text2: `Email verification code sent to ${emailInput}` });
            setStep('verifyCode');
        }else{
            Toast.show({ type: 'error', text1: 'Verification Error', text2: `Verification error, please retry.` });
        }
    };
        ...
        if (res.verified) {
            ...
            if (result.success) {
                ...
                if (mode === 'primary') {
                    ...
                    const updatedRecoveryList = [...(user.recoveryEmails || []), newEntry];
                    dispatch(updateEmailData({ recoveryEmails: updatedRecoveryList }));
                }
                setStep('idle');
            }else{
                Toast.show({ type: 'error', text1: `Update Error`, text2: `${mode} email not updated, please retry.` });
                setStep('idle');
            }
            setCodeInput('');
            setEmailInput('');
        }else{
            Toast.show({ type: 'error', text1: `Verification Error`, text2: `Email not verified, please retry.` });
            setStep('idle');
        }
    };
      ...
      { 
        ...
        onPress: async () => {
          ...
          if (res.success) {
            ...
            Toast.show({ type: 'success', text1: 'Recovery email removed' });
          }
        } 
      }
    ]
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <View style={styles.section}>
                ...
                <TouchableOpacity style={styles.inlineButton} onPress={() => {setStep('primaryInput'); setMode('primary')}}>
                    <Text style={styles.buttonText}>Change</Text>
                </TouchableOpacity>
            </View>
            <View style={styles.section}>
                <Text style={styles.header}>Recovery Emails</Text>
                {user.recoveryEmails ? (user.recoveryEmails || []).map(item => (
                    <>
                        <View style={styles.emailContainer}>
                            <TextInput key={item.email} value={item.email} editable={false} style={[styles.disabledInput, { marginBottom: 8 }]} />
                            <TouchableOpacity onPress={() => handleDeleteRecovery(item.email)}>
                                <MaterialIcons name="delete-outline-outlined" size={16} color={PRIMARY_COLOR} />
                            </TouchableOpacity>
                        </View>
                    </>
                )): (
                ...
                <TouchableOpacity style={styles.inlineButton} onPress={() => {setStep('recoveryInput'); setMode('recovery')}}>
                ...
                <View style={styles.actionCard}>
                    ...
                    {(step === 'primaryInput' || step === 'recoveryInput') && (
                        <>
                            ...
                            <TouchableOpacity style={[styles.inlineButton, {marginTop: 10}]} onPress={handleSendCode}>
                    ...
                    {step === 'verifyCode' && (
                        <View>
                            <Text style={styles.instructionText}>Enter the 6-digit code sent to {emailInput}</Text>
                            <TextInput 
                                style={styles.disabledInput}
                                placeholder="000000" 
                                placeholderTextColor={PRIMARY_COLOR_TINT}
                                ...
                                maxLength={6}
                            />
                            <Text style={[styles.timerText, timer < 60 && { color: PRIMARY_COLOR }]}>
                                Code expires in: {formatSignupTime(timer)}
                            </Text>
                            <TouchableOpacity style={styles.inlineButton} onPress={handleVerify}>
                                <Text style={styles.buttonText}>Verify & Update</Text>
                            </TouchableOpacity>
                        </View>
                    )}
                    <TouchableOpacity onPress={() => setStep('idle')} style={styles.cancelButton}>
