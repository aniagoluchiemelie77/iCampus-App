...
export const ICashBuyPage = ({ navigation }: any) => {
    ...
    const [localCurrencyEquivalent, setLocalCurrencyEquivalent] = useState('0.00');
    ...
    const rawAmount = parseFloat(iCashAmount) * EXCHANGE_RATE_USD * currencyData.rate;
    ...
    useEffect(() => {
        ...
        if (numericICash > 0) {
            ...
            setLocalCurrencyEquivalent(calculatedLocalCurrency.toLocaleString(undefined, {
    ...
    const finalExecution = useCallback(async () => {
        ...
        try {
            ...
            if (!pinRes.ok) {
                Toast.show({ type: 'error', text1: 'Verification Failed', text2: pinData.message });
                return; 
            }
            ...
            if (data.status === 'success') {
                ...
                navigation.navigate('iCashWithdrawSuccessScreen', {
                    amount: iCashAmount,
                    payout: finalPayout,
                    currency: currencyData.code
                });
            ...
            Toast.show({ type: 'error', text1: 'Withdrawal Failed', text2: error.message });
        } finally {
            setIsProcessing(false);
        }
    }, [pin, iCashAmount, finalPayout, fee, currencyData.code, user.uid, selectedMethod, navigation]);
    const verifyPin = async (code: string) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const data = await response.json();
                Toast.show({ type: 'error', text1: 'Invalid PIN', text2: data.message });
            }
        } catch (err) {
            Toast.show({ type: 'error', text1: 'Security Error', text2: 'Connection failed' });
        } finally {
            setIsProcessing(false);
        }
    };
        ...
        <ScrollView style={iCashActionsStyles.container}>
            ...
            <View style={iCashActionsStyles.bodyContainer}>
                ...
                <View style={iCashActionsStyles.inputContainer}>
                    <Icon name="diamond" size={20} color={PRIMARY_COLOR} style={{ marginRight: 8 }} />
                    <TextInput
                ...
                <View style={iCashActionsStyles.exchangeCard}>
                    <View style={iCashActionsStyles.exchangeRow}>
                        ...
                        <Text style={iCashActionsStyles.exchangeValue}>
                            1 iCash ≈ {localCurrency} {(EXCHANGE_RATE_USD * currencyData.rate).toFixed(2)}
                        </Text>
                    </View>
                    ...
                    <View style={iCashActionsStyles.resultDiv}>
                        ...
                        <Text style={iCashActionsStyles.resultValue}>{localCurrencyEquivalent}</Text>
                    </View>
                </View>
                {!hasPaymentMethod && (
                    <View style={iCashActionsStyles.warningBox}>
                        ...
                        <Text style={iCashActionsStyles.warningText}>
                            You haven't added a withdrawal method. Please add a bank account to continue.
                        </Text>
                    </View>
                )}
                <ScrollView
                    ...
                    <View style={iCashActionsStyles.bottomSheet}>
                        <View style={iCashActionsStyles.modalHeader}>
                            <Text style={iCashActionsStyles.modalTitle}>
                                {step === 'details' ? 'Confirm Withdrawal' : 'Enter your iCash PIN'}
                            </Text>
                        ...
                        {step === 'details' ? (
                            <>
                                <View style={iCashActionsStyles.detailRow}>
                                    <Text style={iCashActionsStyles.detailRowText}>Withdraw Amount:</Text>
                                    <View style={iCashActionsStyles.detailRowSubrow}>
                                        <Icon name="diamond" size={19} color='#2222' style={{ marginRight: 4 }} />
                                        <Text style={iCashActionsStyles.detailRowText}>{iCashAmount}</Text>
                                    </View>
                                </View>
                                <View style={iCashActionsStyles.detailRow}>
                                    <Text style={iCashActionsStyles.detailRowText}>Charges (1%):</Text>
                                    <Text style={iCashActionsStyles.detailRowText}> - {localCurrency}{fee.toFixed(2)}</Text>
                                </View>
                                <View style={[iCashActionsStyles.detailRow, iCashActionsStyles.totalRow]}>
                                    <Text style={iCashActionsStyles.totalText}>Amount you will receive:</Text>
                                    <Text style={iCashActionsStyles.totalText}>{localCurrency}{finalPayout.toFixed(2)}</Text>
                                </View>
                               <TouchableOpacity 
                                ...
                                {step === 'pin' && (
                                    <View style={iCashActionsStyles.pinContainer}>
                                        ...
                                        <View style={iCashActionsStyles.pinActionRow}>
                                            <TouchableOpacity onPress={handleBiometricAuth} style={iCashActionsStyles.iconBtn}>
                                                <Icon name="fingerprint" size={28} color={PRIMARY_COLOR} />
                                                <Text style={iCashActionsStyles.iconBtnText}>Use Fingerprint</Text>
                                            </TouchableOpacity>
                                            <TouchableOpacity onPress={handleRequestReset}>
                                                <Text style={iCashActionsStyles.forgotText}>Forgot PIN?</Text>
                                            </TouchableOpacity>
                                        </View>
                                        {isProcessing && <ActivityIndicator style={{ marginTop: 20 }} color={PRIMARY_COLOR} />}
                                    </View>
                                )}
                            </View>
                        )}
