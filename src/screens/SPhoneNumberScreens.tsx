...
import { handleSendWhatsAppCode, verifyPhoneOTPAPI } from '../api/localPostApis.ts';
...
export const PhoneScreen = () => {
    ...
    const sendWhatsappCode = async() => {
        ...
        if (result && result.success){
            ...
            Toast.show({ type: 'error', text2: 'Failed to send code, please retry.' });
            return;
        }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        {user.phoneNumbers ? user.phoneNumbers?.map((item) => (
          <View key={item.number} style={styles.phoneRow}>
             <MaterialIcons name="smartphone-outlined" size={20} color='#222' />
             <Text style={styles.phoneNumberText}>{item.number}</Text>
             {item.isVerified && (
               <MaterialIcons name="verified-user-outlined" size={20} color={PRIMARY_COLOR} style={styles.verifiedIcon} />
             )}
             <TouchableOpacity onPress={() => handleDeleteRecovery(item.number)}>
               <MaterialIcons name="delete-outline" size={20} color={PRIMARY_COLOR} />
             </TouchableOpacity>
          </View>
        )): (
