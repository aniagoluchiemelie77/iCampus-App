...
export const CreateReviewScreen = ({ route, navigation }: any) => {
    ...
    const TARGET_TYPE_CONFIGS: Record<string, { 
    ...
    const config = TARGET_TYPE_CONFIGS[targetType] || TARGET_TYPE_CONFIGS.physical;
    ...
    const updateAttributeRating = (key: keyof ReviewAttributes, value: number) => {
        setAttributes(prev => ({ ...prev, [key]: value }));
    };
    const handleSubmitReview = async () => {
        if (rating === 0) {
            Alert.alert('Rating Required', 'Please choose a primary star rating level.');
            return;
        }
        try {
            ...
            if (localImageUris.length > 0) {
                finalMediaUrls = await Promise.all(localImageUris.map(uri => uploadToFirebase(uri, 'review-attachments')));
            }
            const reviewPayload = {
    ...
    const renderStarRatingRow = (currentVal: number, onChange: (val: number) => void, maxStars = 5) => {
        return (
            <View style={styles.starRow}>
                {[...Array(maxStars)].map((_, i) => {
                    const starIndex = i + 1;
                    return (
                        <TouchableOpacity key={starIndex} onPress={() => onChange(starIndex)} activeOpacity={0.7}>
                            <MaterialIcons
    ...
    const checkAuthentication = async () => {
      try {
        const token = await AsyncStorage.getItem('accessToken') || route.params?.authFallback;
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
      ...
      <View style={styles.block}>
        ...
        {attributes.accuracy !== undefined && (
          <View style={styles.attributeRow}>
            ...
            <Text style={styles.attributeDescription}>
                How well did the product description, files, or service details match what you actually received?
            </Text>
            {renderStarRatingRow(attributes.accuracy, (val) => updateAttributeRating('accuracy', val))}
          </View>
        )}
        ...
        {attributes.deliverySpeed !== undefined && (
          <View style={styles.attributeRow}>
            ...
            <Text style={styles.attributeDescription}>
                Rate the speed of delivery drop-off, agent handling times, or download availability.
            </Text>
            {renderStarRatingRow(attributes.deliverySpeed, (val) => updateAttributeRating('deliverySpeed', val))}
          </View>
        )}
        ...
        {attributes.clarity !== undefined && (
          <View style={styles.attributeRow}>
            ...
            <Text style={styles.attributeDescription}>
                Evaluate the video resolution, audio quality, instruction quality, or explanation breakdown.
            </Text>
            {renderStarRatingRow(attributes.clarity, (val) => updateAttributeRating('clarity', val))}
          </View>
        )}
      </View>
      <View style={styles.block}>
      ...
      <View style={styles.block}>
        <Text style={styles.blockHeading}>Media Uploads ({localImageUris.length}/3)</Text>
        <View style={styles.mediaContainerStrip}>
          {localImageUris.map((uri, index) => (
            <View key={index} style={styles.imagePreviewWrapper}>
              <Image source={{ uri }} style={styles.previewThumbnail} />
              <TouchableOpacity style={styles.closeBadgeButton} onPress={() => removeSelectedImage(index)}>
          ...
          {localImageUris.length < 3 && (
            <TouchableOpacity style={styles.addPhotosBoxButton} onPress={handlePickImages}>
