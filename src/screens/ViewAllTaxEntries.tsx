...
export const AllTaxEntriesScreen = () => {
  ...
  const years = Array.from({ length: Math.max(5, currentYear - 2025 + 1) }, (_, index) => {
  ...
  const renderItem = ({ item }: { item: TaxEntry }) => {
    const handlePress = () => {
      if (item.taxType !== 'product_tax' && item.sourceDetails?.relatedTransactionId) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <PageHeader
        ...
        showBackButton={true}
        rightElement={
          <TouchableOpacity onPress={handleDownloadSubmit} style={[styles.downloadButton, {backgroundColor: colors.btnColor}]}>
            <Text style={[styles.downloadButtonText, {color: colors.btnTextColor}]}>Download</Text>
            <MaterialIcons
      ...
      />
      {isLoading ? (
        <View style={[styles.loaderContainer, {backgroundColor: colors.backgroundSecondary}]}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.backgroundSecondary }]}>
            <Text style={[styles.modalTitle, { color: colors.textDarker }]}>Select Period to Download</Text>
  ...
  >
    {years.map((y) => (
      <Picker.Item key={y.value} label={y.label} value={y.value} />
            ...
            {/* Action Button */}
              <TouchableOpacity
                style={[styles.actionButton, { backgroundColor: colors.btnColor }]}
              ...
              >
                {isDownloading ? (
                  ...
                  <Text style={[ styles.actionButtonText, { color: colors.btnTextColor }]}>Download</Text>
                )}
              </TouchableOpacity>
          </View>
        </View>
